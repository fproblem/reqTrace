...
поведение осознанно — обнови и тесты: они фиксируют правило статусов (v1.5.8,
поведение inline-комментариев Confluence): привязка показывается при точном
совпадении текста (различия в пробелах/вёрстке, невидимые символы и одна вставка
внутрь выделения правкой не считаются); правка/удаление ЧАСТИ цитаты →
подсвечиваются уцелевшие куски строго в ЯКОРНОМ блоке и статус «Требует
проверки»; уцелело меньше половины цитаты или её блок удалён целиком →
«Утрачено» (при удалении блока проекция обнуляет якоря — иначе съехавший индекс
цеплял частичным совпадением соседний похожий пункт, регрессия §6). Частичный
поиск по всей странице запрещён.
