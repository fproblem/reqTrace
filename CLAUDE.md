...
**Перед открытием любой новой ветки — сверься с origin (обязательно).**
Релизы выходят и с других машин, локальный `main` может отставать. Порядок:
`git fetch origin` → если `git log main..origin/main --oneline` не пуст —
обновить main (`git checkout main && git merge --ff-only origin/main`) →
убедиться, что верхний блок `changelog.json` совпадает с последним тегом
`v.*` (`git tag --list 'v.*' | sort -V | tail -1`) — и только потом ветвиться
и выбирать следующий номер версии. Урок v1.7.5: ветку открыли от несвежего
main и «не увидели» уже выпущенный v1.7.4.
