...
- **Идеи по улучшению ReqTrace — сразу в `BACKLOG.md`.** Если в обсуждении или
  по ходу работы проскакивает идея/недоделка/наблюдение «стоило бы сделать» —
  запиши её в бэклог (раздел по масштабу, одна строка «зачем»), даже если сейчас
  она не в работе. Пользователь периодически возвращается к этому файлу и
  выбирает, что делать дальше. Отклонённые идеи — в раздел «Отклонено» с
  причиной, чтобы не предлагать заново.
