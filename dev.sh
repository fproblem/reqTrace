...
#   ./dev.sh --fresh    # сначала убить старые контейнеры, поднять заново
#
...
#      порт 3000 свободен (иначе npm start умирает молча).
...
#      С --fresh перед этим гасит и удаляет старые контейнеры (docker compose
#      down) — данные БД живут в volume и остаются целы.
...
for arg in "$@"; do
  case "$arg" in
    --fresh) FRESH=1 ;;
    *) fail "Неизвестный аргумент «${arg}». Поддерживается только --fresh (пересоздать контейнеры с нуля)." ;;
  esac
done
...
# Занятый 3000 валит npm start молча (CRA без терминала не спрашивает про
# другой порт) — лучше честный отказ до тяжёлых шагов.
if lsof -nP -iTCP:3000 -sTCP:LISTEN >/dev/null 2>&1; then
  fail "Порт 3000 уже занят — похоже, npm start работает в другом терминале. Остановите его (Ctrl+C) и повторите."
fi
note "docker работает, .env на месте, порт 3000 свободен, роль — полигон"
...
if [ "$FRESH" = "1" ]; then
  say "2/4 Пересоздаю контейнеры с нуля (--fresh): гашу старые, поднимаю свежие"
  # down удаляет контейнеры, но НЕ volume pgdata — данные БД целы.
