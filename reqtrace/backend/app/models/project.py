...
class ProjectCredential(Base):
    ...
    # Личный Jira-токен (PAT, Bearer) — ТОЛЬКО для чтения названий тестов
    # (v1.7.0, план — jira-test-names-plan-v1.7.md). Опционален: без него фича
    # молчит. Шифруется тем же Fernet(CREDENTIALS_KEY), что пароль Confluence.
    jira_token_enc: Mapped[str | None] = mapped_column(Text, nullable=True)
    # ok | invalid; NULL = токена нет. 401 от Jira метит invalid — ночью
    # invalid не перепроверяется (та же осторожность, что с кредами Confluence).
    jira_token_status: Mapped[str | None] = mapped_column(String(16), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
