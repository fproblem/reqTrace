...
def _entries_for_run(
    ...
    if run.status == "skipped":
        ...
        # БЕЗ return: связь могла оборваться ПОСРЕДИ прогона (v1.6.4) — к
        # этому моменту переходы уже применены к привязкам, и повторный добор
        # их «не увидит» (страницы уже обновлены). Дайджест обязан выйти из
        # этой же строки журнала — иначе изменения пропадут из колокольчика.
