export const colors = {
  white: '#FFFFFF',
  // Полотно углублено с #F8F9FA (ревью островов v1.8.0): на почти белом фоне
  // острова не «светились», гэпы и тени читались слабо.
  background: '#F1F3F5',
  greenAccent: '#7AE05A',
  greenDark: '#4DB830',
...
export const island = {
  ...
  // Иерархия глубины (ревью): служебные острова (сайдбар, бары, панель
  // привязки) — тихая карточная тень…
  boxShadow: shadows.card,
  // …а герой-поверхность (остров СОДЕРЖИМОГО: контент страницы, экран
  // виртуальной, заглушка «/») приподнята — «рабочая поверхность ближе к
  // читателю». Первая версия (4/16 0.06) была ниже порога заметности на
  // светлом полотне (ревью «не вижу разницы») — подъём сделан честно
  // видимым: глубокое смещение вниз + мягкий ореол. Кажется грязно —
  // крутить альфы здесь, обе тени в одном месте.
  boxShadowRaised: '0 8px 24px rgba(0, 0, 0, 0.11), 0 2px 6px rgba(0, 0, 0, 0.06)',
} as const;
